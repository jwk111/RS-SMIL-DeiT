# copypaste FROM: https://github.com/facebookresearch/dino
...
def load_pretrained_weights(model, pretrained_weights, checkpoint_key):
    ...
    # print(model.load_state_dict(weights_dict['model'], strict=False))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
        ...
        self.num_classes = num_classes      #my add
...
def deit_tiny(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    ...
    if pretrained:
        checkpoint = download_weights("deit_tiny", patch_size, kwargs['pretrained_type'])
        model.load_state_dict(checkpoint)    
    return model


def deit_small(patch_size=16, img_size=[224], pretrained=False,ssp_path=None,num_classes=21, **kwargs):
    model = VisionTransformer(img_size=img_size,
        patch_size=patch_size, num_classes=num_classes,embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    if pretrained:
        # checkpoint = download_weights("deit_small", patch_size, kwargs['pretrained_type'])
        if ssp_path:
            checkpoint = ssp_path
            load_pretrained_weights(model,checkpoint,'teacher')
        else:
            checkpoint = '/home/jwk/Project/remoteSensing/RS-SMIL-DeiT/checkpoints/deit_small_patch16_224-cd65a155.pth'
        # model.load_state_dict(checkpoint)
            weights_dict = torch.load(checkpoint, map_location='cuda:0')
            # 删除不需要的权重
            del_keys = ['head.weight', 'head.bias']
...
def vit_base(patch_size=16, img_size=[224], pretrained=False, **kwargs):
    model = VisionTransformer(img_size=img_size,
        patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    if pretrained:
        checkpoint = download_weights("vit_base", patch_size, kwargs['pretrained_type'])
        model.load_state_dict(checkpoint['model'])    
    return model


def MIL_deit_small(patch_size=16, img_size=[224], pretrained=False,ssp_path=None,num_classes=21,  **kwargs):
    model = MILVisionTransformer(img_size=img_size,
        patch_size=patch_size, num_classes=num_classes,embed_dim=384, depth=12, num_heads=6, mlp_ratio=4,
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    if pretrained:
        # checkpoint = download_weights("deit_small", patch_size, kwargs['pretrained_type'])
